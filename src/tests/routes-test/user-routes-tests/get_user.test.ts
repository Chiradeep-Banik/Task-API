...
describe('Test for GET at /users/me',()=>{
    describe('Auth token is not provided',()=>{
        test('Should return 400',async ()=>{
            var res = await supertest(app).get('/users/me');
            expect(res.status).toBe(400);
        });
    });
    describe('Auth token is provided',()=>{
        test('Should return 400 -- for wrong token',async ()=>{
            var res = await supertest(app).get('/users/me').set('Authorization','Bearer wrong_token');
            expect(res.status).toBe(400);
        });
        describe('Everything is correct',()=>{
            test('Should return 200',async ()=>{
                var res = await supertest(app).get('/users/me').set('Authorization',`Bearer ${token}`);
